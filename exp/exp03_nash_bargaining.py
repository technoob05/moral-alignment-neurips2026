# ============================================================================
# EXP03: NASH BARGAINING AGGREGATION (Training-Free)
# ============================================================================
# IDEA: Each cultural persona = a player in a cooperative game.
#       Nash Bargaining Solution = geometric mean of probability distributions.
#       This ensures FAIRNESS: no single culture dominates.
#       Adaptive bargaining power based on agent confidence.
#
# z_nash(v) = prod_i p_i(v)^w_i / Z  (normalized geometric mean)
# In log-space: log z(v) = sum_i w_i * log p_i(v) - log Z
#
# REF: Nash (1950); Game-theoretic LLM alignment (IJCAI 2025)
# ============================================================================
...
WORK_DIR = Path("/kaggle/working/EXP03_NASH")
...
METHODS_TO_EVAL = ["VANILLA", "NASH_BARGAINING"]

@dataclass
class NashConfig:
    tau: float = 1.0                 # softmax temperature
    tau_bargain: float = 0.5         # bargaining power temperature
    use_adaptive_power: bool = True  # weight by confidence
    min_prob_clip: float = 1e-8      # clip to avoid log(0)
    jsd_threshold: float = 0.2       # high JSD -> increase tau for democracy
    tau_jsd_scale: float = 3.0       # how much to increase tau with JSD
...
# ============================================================================
# CORE: NASH BARGAINING AGGREGATION
# ============================================================================
def query_llm_nash(tokenizer, model, prompts, lang="en",
                   max_new_tokens=MAX_NEW_TOKENS, device=DEVICE, cfg=CFG):
    """
    Nash Bargaining Aggregation:
    Each persona is a "player". The Nash Bargaining Solution maximizes
    the product of utilities = geometric mean of distributions.

    In log-space: log q(v) = sum_i w_i * log p_i(v) - log Z
    where w_i = bargaining power (confidence-based)

    When JSD is high (cultural disagreement), tau increases -> more democratic.
    When JSD is low (consensus), confident agents dominate.
    """
    if not prompts: return [], {}
    ...
    diag = {"jsd_per_step":[],"tau_per_step":[],"mppi_triggered_steps":0,"total_steps":0,"power_entropy":[]}
    ...
    with torch.no_grad():
        for step in range(max_new_tokens):
            ...
            for b in range(B):
                ...
                # Compute bargaining power (confidence-based)
                if cfg.use_adaptive_power:
                    H = _entropy(agent_logits)  # (N,)
                    # Adaptive tau: high JSD -> high tau -> more equal power
                    tau_t = cfg.tau_bargain * (1.0 + cfg.tau_jsd_scale * jsd_t)
                    w = torch.softmax(-H / tau_t, dim=-1)  # (N,) low entropy = high power
                else:
                    w = torch.ones(N, device=device) / N
                    tau_t = cfg.tau_bargain

                # Nash Bargaining: geometric mean in log-space
                # log q(v) = sum_i w_i * log p_i(v)
                log_probs = torch.log_softmax(agent_logits / cfg.tau, dim=-1)  # (N, V)
                log_probs = log_probs.clamp(min=math.log(cfg.min_prob_clip))

                # Weighted geometric mean in log-space
                log_nash = (w.unsqueeze(-1) * log_probs).sum(dim=0)  # (V,)
                # No need to normalize since we argmax (but we can for proper prob)

                diag["jsd_per_step"].append(jsd_t)
                diag["tau_per_step"].append(tau_t)
                z_agg_list.append(log_nash)
...
# ============================================================================
# BOILERPLATE (parse, eval, AMCE, viz, main) - same structure as other exps
# ============================================================================
...
def run_language_eval(lang,tokenizer,model,method,max_rows=None):
    ...
    for start in tqdm(range(0,len(df),BATCH_SIZE),desc=f"{lang}/{method}"):
        ...
        else: raw,diag=query_llm_nash(tokenizer,model,prompts,lang=lang)
...
METHOD_COLORS={"Human":"#2C3E50","VANILLA":"#E74C3C","NASH_BARGAINING":"#E67E22"}
METHOD_LABELS={"VANILLA":"Vanilla","NASH_BARGAINING":"Nash Bargaining (Ours)"}
def plot_radar(mp,hl):
    ...
    ax.legend(loc="upper right",bbox_to_anchor=(1.35,1.15)); ax.set_title("EXP03: Nash Bargaining",y=1.12,fontsize=13,fontweight="bold")
    plt.tight_layout(); plt.savefig(FIGS_DIR/"radar_exp03.png",dpi=150,bbox_inches="tight"); plt.show()
